...
public class GameView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public GameView(Context context, int width, int height, long tiempo) {
        ...
        offset = -height / 48;
        r = new Rectangulo(1, width / 3, 6 * height / 7 + offset, width / 3 + width / 45, height / 8);
        // CACA
        paint.setARGB(255, 255, 255, 255);
        sharedPref = getContext().getSharedPreferences(getContext().getString(R.string.sharedPoints), Context.MODE_PRIVATE);
        ...
        background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.botona));
        botona = Bitmap.createScaledBitmap(background, width / 3, height / 8, true);
        background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.fondo_lienzo));
        lienzoBg = Bitmap.createScaledBitmap(background, width, height, true);
        if (resistencia) {
            background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.instrucr));
            instruc = Bitmap.createScaledBitmap(background, width, height, true);
            background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.instrucr2));
            instruc2 = Bitmap.createScaledBitmap(background, width, height, true);
        } else {
            background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.instrucn));
            instruc = Bitmap.createScaledBitmap(background, width, height, true);
            background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.instrucn2));
            instruc2 = Bitmap.createScaledBitmap(background, width, height, true);
        }
        background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.puntos));
        ...
        background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.porcentaje));
        porcentaje = Bitmap.createScaledBitmap(background, ancho_digito, alto_digito, true);
        background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.fintiempo));
        fintiempo = Bitmap.createScaledBitmap(background, width, height, true);

        ancho_digito = width / 16;
        alto_digito = height / 14;

        background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.nivel_0));
        ncero = Bitmap.createScaledBitmap(background, ancho_digito, alto_digito, true);
        background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.nivel_1));
        nuno = Bitmap.createScaledBitmap(background, ancho_digito, alto_digito, true);
        background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.nivel_2));
        ndos = Bitmap.createScaledBitmap(background, ancho_digito, alto_digito, true);
        background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.nivel_3));
        ntres = Bitmap.createScaledBitmap(background, ancho_digito, alto_digito, true);
        background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.nivel_4));
        ncuatro = Bitmap.createScaledBitmap(background, ancho_digito, alto_digito, true);
        background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.nivel_5));
        ncinco = Bitmap.createScaledBitmap(background, ancho_digito, alto_digito, true);
        background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.nivel_6));
        nseis = Bitmap.createScaledBitmap(background, ancho_digito, alto_digito, true);
        background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.nivel_7));
        nsiete = Bitmap.createScaledBitmap(background, ancho_digito, alto_digito, true);
        background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.nivel_8));
        nocho = Bitmap.createScaledBitmap(background, ancho_digito, alto_digito, true);
        background = drawableToBitmap(ContextCompat.getDrawable(getContext(), R.drawable.nivel_9));
        ...
        //arrayColor = new int[1];
        //arrayColor[0] = 0x88000000 | (0x000000ff << level);
    }
    ...
    public boolean onTouchEvent(@NonNull MotionEvent event) {
        Pair<Integer, Integer> aux = new Pair<>((int) event.getX(), (int) event.getY());

        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (partida_ON) {
                    lastPair = aux;
                    last_onMove = SystemClock.uptimeMillis();
                }
                if (fin_finpartida) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (partida_ON) {
                    if (SystemClock.uptimeMillis() - last_onMove > 50) {
                        cola_click.add(aux);
                    }
                    else {
                        Log.d("Roger ahorro","Nos hemos ahorrado 1 en la cola");
                    }
                }
                break;
    ...
    public void render(Canvas canvas) {
        ...
        if (!partida_ON) {
            if (cambio) {
                canvas.drawBitmap(instruc2, 0, 0, null);
            } else {
                canvas.drawBitmap(instruc, 0, 0, null);
            }
        }
        ...
        if (fin_partida) {
            if (!fin_finpartida) {
                canvas.drawBitmap(fintiempo, 0, 0, null);
            } else {
                canvas.drawBitmap(resultado, 0, 0, null);
                if (apretado) {
                    canvas.drawBitmap(botona, width / 3 + width / 45, 6 * height / 7 + offset, null);
                } else {
                    canvas.drawBitmap(boton, width / 3 + width / 45, 6 * height / 7 + offset, null);
                }
                pinta_level(canvas);
        ...
        if (!fin_partida) {
            ...
            if (partida_ON) {
                ...
                for (int ii = 0; ii < cola_click.size(); ++ii) {
                    ...
                    //lienzo.setPixels(arrayColor, 0, 0, x - ancho_pinzel, y - alto_pinzel, ancho_pinzel, alto_pinzel);
                    ...
                    if (lastPair.first > cola_click.get(ii).first) {
                        ...
                        // esquina buena es lainversa(positiva, negativa)
                        if (lastPair.second > cola_click.get(ii).second) {
                            for (int i2 = 0; i2 < pasos; ++i2) {
                        ...
                        } else if (lastPair.second < cola_click.get(ii).second) {// esquina buena es la inversa (positiva, positiva)
                            for (int i2 = 0; i2 < pasos; ++i2) {
                ...
                if (pixels >= 0.9 * total_pixels) {
                    ...
                    editor.putInt(getContext().getString(R.string.lrt), Math.min(sharedPref.getInt(getContext().getString(R.string.lrt), Integer.MAX_VALUE), (int) (SystemClock.uptimeMillis() - (last_time))));
                    last_time = SystemClock.uptimeMillis();
                    beginTime = Math.min(SystemClock.uptimeMillis(), beginTime + tiempo_regeneracion);
                    level++;
                }
    ...
    private void guardar_puntos() {
        ...
        if (resistencia) {
            editor.putInt("puntos_resistencia_aux", Math.max(sharedPref.getInt("puntos_resistencia_aux", 0), (int) (level * 100 + (pixels * 100) / (0.9 * total_pixels))));
            ...
            editor.putInt("puntos_normal_aux", Math.max(sharedPref.getInt("puntos_normal_aux", 0), (int) (level * 100 + (pixels * 100) / (0.9 * total_pixels))));
    ...
    private void displayLevel(Canvas canvas, int level) {
        ...
        if (dig1 != 0 || dig2 != 0) {
            dibuja_ndigito(canvas, dig2, posx, posy);
        }
        posx += width / 16 - width / 96;
        posy -= width / 32;
